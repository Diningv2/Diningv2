...
class MainView extends Component {
  ...
  favoritesTabButtons = [this.servedToday, this.allFavorites].map(tabName => {
    return {
      tabName: tabName,
      function: () => this.setState({ selectedTab: tabName })
    };
  });

  displayContent = () => {
    if (!this.props.userInformation.notificationID) {
      // No permission for notifications
      return (
        <CenterTextView message="Enable push notifications to allow you to favorite dishes!" />
      );
    } else if (this.props.favoritesList.isLoading) {
      return <LoadingIndicator />;
    } else if (
      // The list is done loading but you don't have any favorites
      ...
      return (
        <View style={{ flex: 1 }}>
          ...
          {this.state.selectedTab == this.allFavorites && <FavoritesListAll />}
