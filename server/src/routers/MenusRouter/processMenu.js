 ...
 *
 *   Summary:
 *       Transforms the response returned by the Yale Dining API for a single location into a list of either Menus objects or MenuItem objects
 *
 *   Parameters:
 *       response - the object returned by the Yale Dining API for a specific location
 ...
 */
export default function processMenu(response) {
    // throw on bad response from Yale Dining
    if (!response || !response.data || !response.data.DATA || 
        !response.data.DATA.length || !response.data.DATA[0].length) {
