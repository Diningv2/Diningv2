...
function getMenuItemList(columns, data) {
  ...
  const menu = data.map(entry => {
    return !entry || !entry.length ? undefined : {
      name: entry[columns.indexOf("MENUITEM")],
      itemID: entry[columns.indexOf("MENUITEMID")]
    };
  }); // filter out undefined and duplicate entries

  const filteredMenu = menu.filter(entry => entry != undefined).filter((entry, index, self) => index === self.findIndex(otherEntry => otherEntry.name === entry.name && otherEntry.itemID === entry.itemID));
