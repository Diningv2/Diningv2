...
async function getAllMenus(query) {
  ...
  for (let location in _locations.default) {
    try {
      ...
      nErrors++;
      console.warn(e.message + ": " + _locations.default[location] + " (total: " + nErrors + ")");
  ...
  if ("meal" in query) // filter out duplicate entries from the list of MenuItems
    return allMenus.filter((entry, index, self) => self.findIndex(otherEntry => otherEntry.name === entry.name && otherEntry.itemID === entry.itemID) === index);else return allMenus;
}