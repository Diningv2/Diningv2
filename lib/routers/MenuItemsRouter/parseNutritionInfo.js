...
function parseNutritionInfo(data) {
  if (!data || data.length == 0) return null; // pad 0s to hanging decimal points
      ...
      if (data[i][c] == ".") {
        if (c != 0) {
          if (isNaN(parseInt(data[i][c - 1]))) {
            data[i] = data[i].substr(0, c) + "0" + data[i].substr(c, data[i].length);
          } else if (isNaN(parseInt(data[i][c + 1]))) {
            data[i] = data[i].substr(0, c + 1) + "0" + data[i].substr(c + 1, data[i].length);
          }
        } else {
          data[i] = "0" + data[i];
        }
